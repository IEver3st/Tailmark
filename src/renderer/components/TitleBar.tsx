...
export function TitleBar(): React.JSX.Element {
  const sleepsToTray = useAppStore((state) => state.snapshot?.settings.keepRunningInTray ?? false);
  return <header className="titlebar">
    ...
    <div className="window-controls">
      ...
      <button
        type="button"
        className="close"
        aria-label={sleepsToTray ? 'Sleep in Windows notification area' : 'Close Tailmark'}
        title={sleepsToTray ? 'Sleep in notification area' : 'Close Tailmark'}
        onClick={() => void window.tailmark.window.control('close')}
      ><CloseIcon /></button>
    </div>
  </header>;
}
