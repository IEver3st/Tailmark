import { AlertTriangle, CheckCircle2, Download, FolderCog, FolderOpen, Moon, Palette, RefreshCw, RotateCcw, Trash2 } from 'lucide-react';
...
export function SettingsPage(): React.JSX.Element {
  ...
  if (!snapshot) return <main className="page snapshot-loading" aria-live="polite"><span className="spinner" aria-hidden="true" /><strong>Loading settings…</strong><span>Detecting War Thunder and reading Tailmark preferences.</span></main>;
  ...
  const chooseDownloadsFolder = async () => {
    const result = await window.tailmark.dialogs.chooseDownloadsFolder();
    if (result.ok && result.data) {
      await update({ downloadsFolder: result.data });
      await refresh();
    } else if (!result.ok) {
      showNotice({ kind: 'error', title: 'Watch folder was not changed', detail: result.error?.message ?? 'Choose another folder.' });
    }
  };
  return <main className="page settings-page"><div className="page-toolbar"><div><h1>Settings</h1><span>Appearance, game location, collision safety, background automation, backups, and sound behaviour.</span></div></div><div className="settings-workspace">
    <nav className="settings-nav" aria-label="Settings sections"><a href="#appearance">Appearance</a><a href="#game">War Thunder</a><a href="#imports">Imports</a><a href="#downloads">Downloads</a><a href="#backups">Backups</a><a href="#sound">Sound Mods</a><a href="#storage">Storage</a></nav>
    <div className="settings-content">
      ...
      <section id="imports"><header><FolderCog /><div><h2>Import behaviour</h2><p>Control content duplicates, destination collisions, and source ZIP cleanup.</p></div></header><SettingToggle label="Ignore duplicate content" detail="Skip skin folders and sound packages whose files are already installed, even when their names differ." checked={settings.ignoreDuplicateContent} onChange={(value) => void update({ ignoreDuplicateContent: value })} /><label className="setting-select"><span><strong>Existing folder behaviour</strong><small>Used for name collisions. Replacement and merge create a backup first.</small></span><select value={settings.defaultDuplicateBehaviour} onChange={(event) => void update({ defaultDuplicateBehaviour: event.target.value as CollisionPolicy })}><option value="skip">Skip existing folder</option><option value="replace">Replace after backup</option><option value="merge">Merge after backup</option><option value="copy">Install as readable copy</option></select></label><SettingToggle label="Move source ZIP to Recycle Bin" detail="After a skin installs or a downloaded ZIP is confirmed as duplicate, move the source archive to the Recycle Bin. Failed archives are kept." checked={settings.deleteSourceZipAfterInstall} onChange={(value) => void update({ deleteSourceZipAfterInstall: value })} /><SettingToggle label="Confirm before replacement" detail="Require an explicit review before batch replacement operations." checked={settings.confirmBeforeReplacement} onChange={(value) => void update({ confirmBeforeReplacement: value })} /></section>
      <section id="downloads"><header><Download /><div><h2>Downloads automation</h2><p>Watch a folder in the background and install only completed, high-confidence user-skin ZIPs.</p></div></header><div className={`path-status ${snapshot.downloadAutomation.status === 'error' ? 'invalid' : 'valid'}`}>{snapshot.downloadAutomation.status === 'error' ? <AlertTriangle /> : <CheckCircle2 />}<div><strong>{snapshot.downloadAutomation.status === 'off' ? 'Off' : snapshot.downloadAutomation.status.replaceAll('-', ' ')}</strong><code>{snapshot.downloadAutomation.folder}</code><span>{snapshot.downloadAutomation.error ?? (snapshot.downloadAutomation.lastEvent ? `${snapshot.downloadAutomation.lastEvent.filename}: ${snapshot.downloadAutomation.lastEvent.detail}` : 'Sounds, mixed archives, conflicts, and uncertain downloads are left for review.')}</span></div></div><div className="setting-actions"><button type="button" onClick={() => void chooseDownloadsFolder()}><FolderOpen />Choose watch folder</button>{settings.downloadsFolder ? <button type="button" onClick={() => void update({ downloadsFolder: null })}><RotateCcw />Use Windows Downloads</button> : null}{settings.keepRunningInTray ? <button type="button" onClick={() => void window.tailmark.window.control('close')}><Moon />Sleep to tray now</button> : null}</div><SettingToggle label="Automatically install downloaded skins" detail="Wait for each ZIP to finish downloading, scan completed files in parallel, then install only skin-only archives at 85% confidence or higher. Safe Mode and pending recovery pause the watcher." checked={settings.autoInstallDownloads} onChange={(value) => void update({ autoInstallDownloads: value })} /><SettingToggle label="Keep automatic installs running in the tray" detail="Closing Tailmark unloads the window to save resources while the lightweight watcher remains in the Windows notification area. Click its tray icon to reopen Tailmark or use its menu to quit." checked={settings.keepRunningInTray} onChange={(value) => void update({ keepRunningInTray: value })} /></section>
